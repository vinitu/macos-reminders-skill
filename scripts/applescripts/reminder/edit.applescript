on run argv
    ...
    if (count of args) is less than 4 then error "Usage: osascript scripts/applescripts/reminder/edit.applescript <list-name> <reminder-name> <property> <value> [--format=plain|json]"
    ...
    set reminderId to my resolveReminderId(listName, reminderName)
    set updatedValue to my updatePropertyById(listName, reminderId, propertyName, propertyValue)

    if outputFormat is "json" then
        return "{\"status\":\"updated\",\"list\":\"" & my jsonEscape(listName) & "\",\"reminder\":\"" & my jsonEscape(reminderName) & "\",\"id\":\"" & my jsonEscape(reminderId) & "\",\"property\":\"" & my jsonEscape(propertyName) & "\",\"value\":\"" & my jsonEscape(updatedValue) & "\"}"
    end if

    return updatedValue
end run

on resolveReminderId(listName, reminderName)
    tell application "Reminders"
        set targetList to first list whose name is listName
        return id of first reminder of targetList whose name is reminderName
    end tell
end resolveReminderId
