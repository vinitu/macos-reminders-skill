on run argv
    ...
    if (count of args) is less than 2 then error "Usage: osascript scripts/reminder/due-range.applescript <start-date-text> <end-date-text> [list-name] [--format=plain|json]"

    set startDate to date (item 1 of args)
    set endDate to date (item 2 of args)
    if endDate is less than or equal to startDate then error "End date must be later than start date"
        ...
        repeat with currentList in listCollection
            ...
            repeat with reminderName in reminderNames
                ...
                if requireIncomplete and ((completed of first reminder of currentList whose name is currentName) is true) then
                    ...
                    if dueValue is not missing value and dueValue is greater than or equal to startDate and dueValue is less than endDate then set end of hits to currentName
                end if
            end repeat
        end repeat
    end tell
