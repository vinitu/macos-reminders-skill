on run argv
    ...
    if (count of args) is greater than 1 then error "Usage: osascript scripts/reminder/today.applescript [list-name] [--format=plain|json]"
    ...
    set startOfTomorrow to startOfToday + days

    set listName to missing value
    if (count of args) is 1 then set listName to item 1 of args

    set hits to my collectDueWindow(listName, startOfToday, startOfTomorrow, false)
