on run argv
    ...
    if (count of args) is less than 1 then error "Usage: osascript scripts/reminder/search.applescript <exact-name|id|incomplete|flagged|priority|has-due-date|text> [args] [--format=plain|json]"

    set searchMode to item 1 of args

    if searchMode is "exact-name" then
        if (count of args) is less than 3 then error "Usage: osascript scripts/reminder/search.applescript exact-name <list-name> <reminder-name> [--format=plain|json]"
    ...
    if searchMode is "id" then
        if (count of args) is less than 2 then error "Usage: osascript scripts/reminder/search.applescript id <reminder-id> [--format=plain|json]"
        set reminderIdQuery to my trimText(item 2 of args)
        if reminderIdQuery is "" then error "Reminder id query must not be empty"

        tell application "Reminders"
            set hits to name of (every reminder whose id is reminderIdQuery)
        end tell
    ...
    if searchMode is "priority" then
        if (count of args) is less than 2 then error "Usage: osascript scripts/reminder/search.applescript priority <priority> [list-name] [--format=plain|json]"
    ...
    if searchMode is "text" then
        if (count of args) is less than 2 then error "Usage: osascript scripts/reminder/search.applescript text <query> [list-name] [--format=plain|json]"
