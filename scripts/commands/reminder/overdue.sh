...
if raw=$(try_remindctl_all_or_list_json "$list_name"); then
  ...
  cutoff="$(date +%Y-%m-%d)"
  normalize_reminders_json <<< "$raw" | augment_reminders_json | "$JQ_BIN" --arg cutoff "$cutoff" '
    def local_due_date:
...
if [[ -n "$list_name" ]]; then
  run_reminder_applescript overdue.applescript "$list_name" | augment_reminders_json
else
  run_reminder_applescript overdue.applescript | augment_reminders_json
fi
