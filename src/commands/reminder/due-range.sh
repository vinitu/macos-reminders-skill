#!/usr/bin/env bash
# Output: always JSON (array of reminders due in the date range).
# Prefer: remindctl + jq. Fallback: AppleScript when remindctl is missing.
# Example:
# [
#   {
#     "id": "...",
#     "name": "Task",
#     "list": "List",
#     "body": null,
#     "completed": false,
#     "priority": "none",
#     "due_date": "2026-03-13T12:00:00Z"
#   }
# ]
set -euo pipefail

[[ $# -lt 2 ]] && { echo "Usage: $(basename "$0") <start-date> <end-date> [list-name]" >&2; exit 1; }
[[ $# -gt 3 ]] && { echo "Usage: $(basename "$0") <start-date> <end-date> [list-name]" >&2; exit 1; }
start_date="$1"
end_date="$2"
list_name="${3:-}"
# shellcheck source=src/commands/reminder/_lib/common.sh
source "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/_lib/common.sh"

if [[ -n "$REMINDCTL_BIN" ]]; then
  [[ -n "$JQ_BIN" ]] || { echo "jq required when using remindctl" >&2; exit 1; }
  raw=$(remindctl_all_or_list_json "$list_name")
  normalize_reminders_json <<< "$raw" | "$JQ_BIN" --arg start "$start_date" --arg end "$end_date" '[.[] | select(.completed == false and .due_date != null and (.due_date | .[0:10]) >= $start and (.due_date | .[0:10]) <= $end)]'
  exit 0
fi

exec_reminder_applescript_optional_last_arg due-range.applescript "$list_name" "$start_date" "$end_date"
