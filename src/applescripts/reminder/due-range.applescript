-- Output: JSON array of reminders due in the date range. Usage: <start-date> <end-date> [list-name]
on run argv
    set args to my stripFormatArg(argv)
    if (count of args) is less than 2 then error "Usage: osascript due-range.applescript <start-date> <end-date> [list-name]"
    ...
    tell application "Reminders"
        ...
        repeat with L in listCollection
            ...
            repeat with R in rems
                ...
                if dueVal is not missing value and my matchesDueWindow(dueVal, startDate, endDate) then
